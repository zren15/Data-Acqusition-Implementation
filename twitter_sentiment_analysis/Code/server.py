...
def add_color(tweets):
    ...
    for i in range(len(tweets['tweets'])):
        ...
        tweets['tweets'][i]['color'] =str(colors[color_index])
...
def tweets(name):
    ...
    return_dict = fetch_tweets(api,name)
    add_color(return_dict)
    total_score=[]
    for i in range(len(return_dict['tweets'])):
      total_score.append(return_dict['tweets'][i]['score'])
      Median = round(median(total_score),4)
    return render_template('tweets.html',record=return_dict,Median=Median)
...
def following(name):
    ...
    """
    return_following = fetch_following(api,name)
    return_following = sorted(return_following, key=lambda x:x['followers'],reverse=True)
    return render_template('following.html',name=name,return_following=return_following)
